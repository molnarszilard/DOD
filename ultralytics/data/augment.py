...
class RandomPerspective:
    ...
    def apply_corners(self, corners, M):
        """
        Apply affine to corners and generate new bboxes from corners.

        Args:
            corners (ndarray): list of corners, [num_samples, 500, 2].
            M (ndarray): affine matrix.

        Returns:
            corners (ndarray): list of corners after affine, [num_samples, 500, 2].
            new_bboxes (ndarray): bboxes after affine, [N, 4].
        """
        n, num = corners.shape[:2]
        if n == 0:
            return [], corners

        xy = np.ones((n * num, 3), dtype=corners.dtype)
        corners = corners.reshape(-1, 2)
        xy[:, :2] = corners
        xy = xy @ M.T  # transform
        xy = xy[:, :2] / xy[:, 2:3]
        corners = xy.reshape(n, -1, 2)
        return corners
    ...
    def apply_keypoints(self, keypoints, M):
        ...
        #     visible[out_mask] = 0
        #     return np.concatenate([xy, visible], axis=-1).reshape(n, nkpt, 3)
        # return np.concatenate([xy], axis=-1).reshape(n, nkpt, 2)
    ...
    def __call__(self, labels):
        ...
        new_instances = Instances(bboxes, segments, keypoints, bbox_format="xyxy", normalized=False, hyp=self.hyp)
        # Clip
...
class LetterBox:
    ...
    def __init__(self, new_shape=(640, 640), auto=False, scaleFill=False, scaleup=True, center=True, stride=32,hyp=None):
        """
...
class CopyPaste:
    ...
    def __init__(self, p=0.5,hyp=None) -> None:
        """
...
class Format:
    ...
    def __call__(self, labels):
        ...
        labels["keypoints"] = torch.from_numpy(instances.keypoints) if nl else torch.zeros((nl, instances.keypoints.shape[1],instances.keypoints.shape[2]))
        # if self.return_keypoint and not self.return_obb:
        #     labels["keypoints"] = torch.from_numpy(instances.keypoints) if nl else torch.zeros((nl, 1, 3))
        # if self.return_keypoint and self.return_obb:
        #     labels["keypoints"] = torch.from_numpy(instances.keypoints) if nl else torch.zeros((nl, 1, 2))
        if self.return_obb:
            labels["bboxes"] = (
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    pre_transform = Compose(
        [
            ...
            CopyPaste(p=hyp.copy_paste,hyp=hyp),
            RandomPerspective(
                degrees=hyp.degrees,
