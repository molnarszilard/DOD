...
<p align="center">
  <img src="assets/arch_horz_simplified3_withimage.png"> <br>
</p>
...
[OHD-SJTU occluded dataset]() for validation only

<p align="center">
  <img src="assets/P480_0450_2700_occluded.jpg"> <br>
</p>

[VEDAI dataset]() for validation only

<p align="center">
  <img src="assets/00000027_co.png"> <br>
</p>

[vine leaves dataset]() the contours, and a few sampled leaves from a laboratory environment are also included.
...
Do not forget to have the required `yaml` file for your dataset (e.g., [config for OHD-SJTU](ultralytics/cfg/datasets/custom/OHD-SJTU-L.yaml)).
...
For OHD-SJTU you can use the given script in their Developement tools, to spit the images into smaller images. The VEDAI dataset is slightly differently organized, for that use the [vedai2dobb.py](data_processing_scripts/vedai2dobb.py) script.

There are also a few other usefule scripts:

If you want to check that your annotations are correct, you can use the [plot_direction.py](data_processing_scripts/plot_direction.py) script.

You might want to enlarge the variations on your dataset with flipping your images using thet [flip_images](data_processing_scripts/flip_images.py) script (this also takes care for the labels).

For the OHD-SJTU datasets, you can introduce occlusions onto the image using the [dpt_occlusion](data_processing_scripts/dpt_occlusion.py) script.

To annotate the direction of the leaves, you need MATLAB, with the [label_leaf_orientation_many](data_processing_scripts/matlab/label_leaf_orientation_many.m) script. For other Leaves dataset related question, please open an issue.

## Environment setup

We recommend to use a conda environment for this code. Most necessary packages are in the [requirements.txt](requirements.txt) file. Make sure NOT to install ultralytics from pip, as this repository contains direct modifications into that code, and installing it would create a confusing environment.

## Pre trained models

DOD model trained on the OHD-SJTU-L dataset: [model]()

DOD model trained on the Leaf dataset: [model]()

## Training

For training the DOD method, you should run the [script_train_dod.py](script_train_dod.py) script. There you can choose the dataset (path to the config file), the representation, and the direction loss type.

## Evaluation

For evaluating a model, use the [script_val.py](script_val.py).

If you have inferenced labels, you can find out more using [compare_dod.py](data_processing_scripts/compare_dod.py) script (Note: for this to properly work, you need to have an annotation file from your predictions for each image, however, [script_predict_dod.py](script_predict_dod.py) does not create one if there is no detections for a given image. For this run the [check_label_image](data_processing_scripts/check_label_image.py) script.)

If you have the inference result from the HDDet method, you can convert those types of data into the required type using the [hddet_xywhr2bb](data_processing_scripts/hddet_xywhr2bb.py) script.

## Image rectification

Once you have the necessary dataset for image rectificaiton, you can create rectified images using [rectify_leaves.py](rectify_leaves.py) script.
